...
def msg(text, **kw):
    for cid in CHAT_ID:
        try:
            ...
            try: bot.send_message(cid, re.sub(r'[*_`\[\]]', '', text))
            except: pass
...
def welcome(m):
    if m.chat.id not in CHAT_ID: return
...
def process_zip(url, folder_name=""):
    ...
    for i, video_path in enumerate(video_files, 1):
        ...
        for j, part in enumerate(parts, 1):
            if len(parts) > 1:
                msg(f"  Part {j}/{len(parts)} upload ho raha hai...")
            jazz_drive_upload(part, folder_name)
            clean(part)
...
def process_direct(url, folder_name=""):
    ...
    msg(f"Downloaded! {sz:.1f} MB\nUploading...")
    parts = split_video(result)
    for i, part in enumerate(parts, 1):
        if len(parts) > 1:
            msg(f"Part {i}/{len(parts)} upload ho raha hai...")
        jazz_drive_upload(part, folder_name)
        clean(part)
MAX_SIZE_MB = 1990

def split_video(filepath):
    """1990MB se badi file ko parts mein split karo"""
    size_mb = os.path.getsize(filepath) / (1024 * 1024)
    if size_mb <= MAX_SIZE_MB:
        return [filepath]

    msg(f"File {size_mb:.0f}MB hai — 1990MB parts mein split ho rahi hai...")
    ...
    try:
        ...
        msg("Split fail — duration nahi mila"); return [filepath]
    ...
    for i in range(num_parts):
        ...
        if os.path.exists(part_path) and os.path.getsize(part_path) > 1024:
            ...
            psz = os.path.getsize(part_path) / (1024 * 1024)
            msg(f"Part {i+1}/{num_parts} ready: {psz:.0f}MB")
